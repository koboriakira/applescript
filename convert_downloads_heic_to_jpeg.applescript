...
on run
	try
		...
		set confirmMessage to fileCount & "個のHEICファイルが見つかりました。JPEGに変換して元ファイルを削除しますか？"
		...
		-- 結果を表示
		display dialog successCount & "個のファイルが正常に変換・削除されました。" buttons {"OK"} default button "OK"
...
on convertAndDeleteHEIC(heicFile)
	...
	on error errorMessage
		display dialog "ファイル処理エラー (" & (name of (info for heicFile)) & "): " & errorMessage buttons {"OK"} default button "OK"
...
on open droppedFiles
	try
		...
		if processedCount > 0 then
			display dialog processedCount & "個のファイルが処理されました。" buttons {"OK"} default button "OK"
		else
